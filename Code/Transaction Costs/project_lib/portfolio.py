class Portfolio(object):
    def __init__(self, prices, position, period, tcost):
    ...
    def adjusted_profit(self):
        return (self.prices.pct_change() * self.position.shift(periods=self.period) - self.tcost * self.position.shift(periods=self.period).diff().abs()).sum(axis=1)
    ...
    def adjusted_nav(self, init_capital=None):
        # common problem for most CTAs.
        init_capital = init_capital or 100*self.adjusted_profit.std()
        # We assume we start every day with the same initial capital!
        r = self.adjusted_profit / init_capital
        # We then simply compound the nav!
        # We could also achieve the same by scaling the positions with increasing fundsize...
        return (1+r).cumprod()
